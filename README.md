...
| 8 | Employee Verification — Backend | `employee-verification-system/employee-verification-backend/` | Python + FastAPI | `8000` |
| 9 | Employee Verification — Frontend | `employee-verification-system/frontend/frontend/` | React + Vite | `5179` |
| 10 | AI Robo Interview | `HR_robo/` | Python + FastAPI + Groq | `8001` |
| 11 | Smart Attendance | `smart-Attendance-main/smart-Attendance-main/Smart_Attendance/Smart_Attendance/` | Python + Flask + MongoDB | `5001` |

> Vite assigns ports in launch order (5173, 5174, …). Use the explicit `--port` flags below so every portal always gets the same port.
...
Optional:
- **Ollama** (`OLLAMA_URL`) — only if you use the local-AI features of the main backend.
- **Twilio account** — only for SMS/OTP features.
- **Groq API key** — required for the AI Robo Interview.
...
Or download the ZIP from GitHub → **Code → Download ZIP** → extract it → open a terminal inside the extracted folder. Nothing else in the code needs to change — you only create the `.env` files below.
...
### 3.2 Frontend portals — one file each
...
The main backend auto-creates its tables and seeds the Super Admin account on first start — no manual SQL needed beyond creating the databases.
...
**Python services** — create a virtual environment per service (recommended):
...
# Smart Attendance (Flask, port 5001 to avoid clashing with the Node backend)
cd smart-Attendance-main/smart-Attendance-main/Smart_Attendance/Smart_Attendance/backend
venv\Scripts\activate
python app.py
# If it tries to use port 5000, run instead:
#   set FLASK_RUN_PORT=5001 && flask --app app run    (Windows)
```
...
| Smart Attendance | http://localhost:5001 |
...
7. Login as Super Admin → create Departments, Designations → add Employees → their credentials work on the other portals
...
| `Access Denied` / `Forbidden` toast | Role lacks route access — check `protect([...])` in `backen/modules/**/*.routes.js` |
| Blank portal page | Backend not running, or `VITE_API_BASE_URL` missing/wrong in that portal's `.env` |
| Port already in use | Another service took it — pass a different `--port`, or free the port |
...
| OTP SMS not sending | Twilio vars empty — feature is optional; email OTP still works |
...
```
HRMS/
├── backen/                        # Node.js Express API (MySQL, JWT, Socket.IO)
...
│   ├── employee-verification-backend/   # FastAPI + MySQL
│   └── frontend/frontend/               # React + Vite
├── HR_robo/                       # AI Robo Interview (FastAPI + Groq)
├── smart-Attendance-main/         # Smart Attendance (Flask + MongoDB + face engine)
